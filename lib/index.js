...
Redis.defaults.url = Storage.env('REDIS_URL') || 'redis://localhost:6379/{db}-{env}'.assign({
  db: 'default',
  env: process.env.NODE_ENV || 'development'
});
Redis.defaults.options = {
  client: {
    parser: 'hiredis',
...
Redis.prototype.set = function() {
  ...
  self._set(arguments, function(key_values, options, done, next) {
    var commands = Object.keys(key_values).map(function(k) {
      return ['set', [self.resource(k).trimmed.db, k].join('/'), key_values[k]];
    });
    ...
    self.client
      ...
      .exec(function(err, responses) {
        ...
        results = responses.map(function(response) { return (response === 'OK'); });
...
Redis.prototype.get = function() {
  ...
  self._get(arguments, function(keys, options, done, next) {
    var commands = keys.map(function(k) {
      return ['get', [self.resource(k).trimmed.db, k].join('/')];
    });
...
Redis.prototype.del = function() {
  ...
  self._del(arguments, function(keys, options, done, next) {
    var commands = keys.map(function(k) {
      return ['del', [self.resource(k).trimmed.db, k].join('/')];
    });
...
Redis.prototype.exists = function() {
  var self = this;

  self._exists(arguments, function(keys, options, done, next) {
    var commands = keys.map(function(k) { return ['exists', [self.resource(k).trimmed.db, k].join('/')]; });
    ...
    self.client
      ...
      .exec(function(err, responses) {
        ...
        results = responses.map(function(response) { return !!response; });
